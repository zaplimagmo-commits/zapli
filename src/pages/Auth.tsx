...
import { Zap, Eye, EyeOff, ArrowLeft, CheckCircle2, Loader2 } from 'lucide-react';
    ...
    <div className="min-h-screen flex" style={{ background: '#f8fafc' }}>
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(circle at 30% 70%, #6366f1 0%, transparent 50%), radial-gradient(circle at 80% 20%, #10b981 0%, transparent 40%)',
          }}
        />
        ...
        <div className="relative z-10">
          ...
          <p className="text-indigo-200 mb-8">
            Acesse sua conta e continue transformando contatos em clientes.
          </p>
          <div className="space-y-3">
            {[
              'Prospecção automática via WhatsApp',
              'Follow-up nos dias certos, sem esquecimento',
              'Dashboard com funil em tempo real',
            ].map(f => (
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          ...
          <p className="text-muted-foreground text-sm mb-7">
            Use o e-mail e senha cadastrados para acessar o painel.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {error && (
              <div className="rounded-lg p-3 text-sm text-red-700" style={{ background: '#fef2f2', border: '1px solid #fecaca' }}>
                {error}
              </div>
            )}
...
export function RegisterPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', company: '', segment: '', planId: defaultPlan,
  });
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError('');
    const ok = await register(form.name, form.email, form.password, form.company, form.planId);
    setLoading(false);
          ...
          <div className="flex items-center gap-2 mb-7">
            {[1, 2].map(s => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${step >= s ? 'text-white' : 'text-muted-foreground bg-muted'}`}
                ...
                {s < 2 && (
                  <div
                    className={`flex-1 h-px ${step > s ? '' : 'bg-border'}`}
                    style={step > s ? { background: 'var(--emerald)' } : {}}
                  />
                )}
              </div>
            ...
            {step === 1 ? (
              ...
              <div className="space-y-3">
                {Object.values(PLANS).map(plan => (
                  <label
                    key={plan.id}
                    className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all ${form.planId === plan.id ? 'border-emerald-400' : 'border-border hover:border-primary/30'}`}
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {plan.highlight && (
                          <span className="text-xs font-bold px-1.5 py-0.5 rounded text-white" style={{ background: 'var(--emerald)', fontSize: 9 }}>
                            POPULAR
                          </span>
                        )}
                      </div>
