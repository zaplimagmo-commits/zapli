...
import { Bot, Plus, Zap, MessageCircle, Instagram, Trash2, Info, ArrowRight, Loader2 } from 'lucide-react';
...
export default function AppBot() {
    ...
    try {
      ...
      if (data) {
        setRules(data.map(r => ({
          ...
          notifyMessage: r.notify_message,
  ...
  async function handleDeleteRule(id: string, name: string) {
    ...
    if (ok) {
      await db.from('bot_rules').delete().eq('id', id);
  ...
  async function toggleRule(id: string, currentStatus: boolean) {
    await db.from('bot_rules').update({ is_active: !currentStatus }).eq('id', id);
    setRules(prev => prev.map(r => r.id !== id ? r : { ...r, isActive: !r.isActive }));
  }
  ...
  async function saveRule() {
    ...
    const newRuleData = {
      ...
      trigger: { 
        ...
        keywords: form.triggerType === 'keyword' ? form.keywords.split(',').map(k => k.trim()).filter(Boolean) : undefined 
      },
      action: form.action,
      reply_message: form.replyMessage || null,
      notify_message: form.notifyMessage || null
    };

    const { data, error } = await db.from('bot_rules').insert(newRuleData).select().single();
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.triggerType} onChange={e => setForm(f => ({ ...f, triggerType: e.target.value as any }))}
                  className="w-full h-9 rounded-md border border-input bg-background px-3 text-sm">
            ...
            {form.triggerType === 'keyword' && (
              <div>
                <Label className="text-xs mb-1.5 block">Palavras-chave (separadas por vírgula)</Label>
                <Input placeholder="sim, quero, interesse, valor" value={form.keywords} onChange={e => setForm(f => ({ ...f, keywords: e.target.value }))} />
              </div>
            )}
            <div>
              ...
              <select value={form.action} onChange={e => setForm(f => ({ ...f, action: e.target.value as BotAction }))}
                className="w-full h-9 rounded-md border border-input bg-background px-3 text-sm">
                {Object.entries(ACTION_CFG).map(([id, cfg]) => <option key={id} value={id}>{cfg.label}</option>)}
              </select>
            </div>
            {form.action === 'reply' && (
              <div><Label className="text-xs mb-1.5 block">Mensagem de resposta</Label><Textarea placeholder="Digite a resposta automática..." value={form.replyMessage} onChange={e => setForm(f => ({ ...f, replyMessage: e.target.value }))} /></div>
            )}
            {form.action === 'notify_team' && (
              <div><Label className="text-xs mb-1.5 block">Mensagem da notificação</Label><Input placeholder="Ex: Lead interessado no WhatsApp" value={form.notifyMessage} onChange={e => setForm(f => ({ ...f, notifyMessage: e.target.value }))} /></div>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAdd(false)}>Cancelar</Button>
            <Button onClick={saveRule} className="text-white" style={{ background: 'var(--primary)' }}>Criar Regra</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </AppLayout>
